...
export function AssetClassForm({ assetClasses, onSave }: AssetClassFormProps) {
  ...
  const handleUpdateClass = (id: string, field: keyof AssetClass, value: string) => {
      ...
      <CardContent>
        ...
        <div className="mb-4">
          <p className="text-sm mb-2">
            Total alocado: <span className={totalPercentage > 100 ? 'text-red-500 font-bold' : 'font-bold'}>
              {totalPercentage}%
            </span>
            {totalPercentage > 100 && ' (excede 100%)'}
            {totalPercentage < 100 && totalPercentage > 0 && ` (falta ${100 - totalPercentage}%)`}
          </p>
        </div>

        <div className="space-y-4 mb-6">
          {classes.map(cls => (
            <div key={cls.id} className="flex items-center space-x-2 p-3 border rounded-md">
              <div className="flex-1 space-y-1">
                <div className="flex items-center space-x-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleUpdateClass(cls.id, 'percentage', e.target.value)}
                    className="w-20"
                    placeholder="%"
                  />
                  <Button
